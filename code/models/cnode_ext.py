...
class CNODExtmod(nn.Module):
    def __init__(self, Nc, input_dim, hidden_dim, Delta, corr_time, delta_t, method="euler", extended_ode_mode=False,
                 output_fun="mlp", bridge_ode=False, predict_from_cn=False, uncertainty_mode=False, forecast_mode=False, **kwargs):
        """
        ...
        if self.uncertainty_mode:
            ...
            self.uncertainty_mod = nn.Sequential(nn.Linear(Nc * input_dim + input_dim, hidden_dim), nn.ReLU(),
                                                 nn.Linear(hidden_dim, self.uncertainty_dims), nn.Sigmoid())
        else:
        ...
        self.node = nn.Sequential(
            nn.Linear(input_dim * Nc + input_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim), nn.ReLU(),
            nn.Linear(hidden_dim, input_dim * Nc + input_dim))

    def out_fun(self, h):
        return h[..., :self.input_dim]  # g

    def uncertainty_fun(self, h_in):
        return self.uncertainty_mod(h_in)

    def ode_fun(self, t, cn):
        cn_0 = cn[:, :self.input_dim * self.Nc]  # c
        cn_1 = cn[:, self.input_dim * self.Nc:]  # h
        if self.uncertainty_mode:
        ...
        driver_ode = self.node(cn_1)  # dh/dt=phi(h)
        ...
        # t2=time.time()
        # print('matmul:', cn_0view.shape, self.A.shape, t2-t1)
    ...
    def integrate(self, cn, eval_times, ode_function=None, **kwargs):
        ...
        h_cn = h_out[..., :self.input_dim * self.Nc]  # c:T*dn
        h_0 = h_out[..., self.input_dim * self.Nc: self.input_dim * self.Nc + self.input_dim]  # h0
        h_x = h_out[..., self.input_dim * self.Nc:]  # h
    ...
    def forward_ode(self, end_time, start_time, cn, eval_mode=False, **kwargs):
        ...
        # c: t*B*dn, pred:t*B*d, h:t*B*(dn+d)
        # t=eval_times-1
    ...
    def forward(self, times, Y, mask, eval_mode=False, bridge_info=None):
        ...
        """
        h = torch.cat(
            (torch.zeros(Y.shape[0], self.input_dim * self.Nc, device=Y.device),
             torch.zeros(Y.shape[0], self.input_dim * self.Nc + self.input_dim, device=Y.device)), -1)
        last_h = torch.cat(
            (torch.zeros(Y.shape[0], self.input_dim * self.Nc, device=Y.device),
             torch.zeros(Y.shape[0], self.input_dim * self.Nc + self.input_dim, device=Y.device)), -1)
        if self.uncertainty_mode:
        ...
        times_traj = []
        tt1=time.time()
        for i_t, ltime in enumerate(times):
            t3=time.time()
            h_out, pred, d_out, eval_times, h_uncertainty, pred_uncertainty = self.forward_ode(
                ltime, current_time, h, eval_mode=eval_mode)
            t4=time.time()
            # print('1step:', t4-t3)
            h_cn = h_out[-1]  # c at obs_time:B*dn
            g_out = d_out[-1]  # h:B*(dn+d)
            y_pred = pred[-1]  # B*d

            preds_list.append(y_pred)
            y_traj.append(pred[1:].permute(1, 0, 2))  # B*(t-1)*d
            ...
            if self.uncertainty_mode:
                ...
                h_no_update = torch.cat((h_cn, g_out), -1)  # B*(2nd+d)
                h_updated = torch.cat((h_cn, Y[:, i_t], h_cn), -1)  # Y:B*T*d
            ...
            if len(mask.shape) == 3:
                ...
                driver = (driver_update * mask[:, i_t][:, None, :] + driver_no_update * (
                        1 - mask[:, i_t][:, None, :])).reshape(driver_update.shape[0], -1)
                ...
                last_h[mask[:, i_t].any()] = h[mask[:, i_t].any()]  # B*(2nd+d)

            current_time = ltime

        tt2=time.time()
        # print('integration time:', tt2-tt1, len(times))
        y_preds = torch.stack(preds_list, 1)  # B*T*d
        if self.uncertainty_mode:
        ...
        y_traj = torch.cat(y_traj, 1)  # B*allT*d
        times_traj = torch.cat(times_traj, 0)  # allT=(eval_times-1)*T

        #print('timetraj:',times_traj)
        #exit()

        if (y_preds.isnan().any()) or (torch.isinf(y_preds).any()):
            import ipdb;
            ipdb.set_trace()
        return (y_preds, y_traj, times_traj, last_h[..., :self.input_dim * self.Nc],
                h[..., -self.uncertainty_dims * self.Nc:], y_uncertainty, y_uncertainty_traj)
...
class CNODExt(pl.LightningModule):
    def __init__(
        ...
        self.node_model = CNODExtmod(Nc=hidden_dim, input_dim=output_dim, hidden_dim=hidden_dim, Delta=Delta,
                                     corr_time=corr_time, delta_t=delta_t,
                                     method=method, extended_ode_mode=extended_ode_mode, output_fun=output_fun,
                                     bridge_ode=bridge_ode, uncertainty_mode=uncertainty_mode, forecast_mode=forecast_mode, **kwargs)
    ...
    def predict_step(self, batch, batch_idx):
        ...
        if self.uncertainty_mode:
            ...
            uncertainty_recs = [np.polynomial.legendre.legval(
                (2 / self.Delta) * (rec_span - Tmax).cpu().numpy() + 1,
                (uncertainty_embedding[..., out_dim].cpu().numpy() * [(2 * n + 1) ** 0.5 for n in range(Nc)]).T) for
                out_dim in range(self.output_dim)]
            uncertainty_recs = torch.Tensor(np.stack(uncertainty_recs, -1))

        return {"Y_future": Y_future, "preds": preds, "mask_future": mask_future, "uncertainty_recs": uncertainty_recs,
                "uncertainty_pred": uncertainty_pred, "pred_rec": recs, "Y_rec": Y[:, mask_rec, :],
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {"Y": Y, "preds": preds, "T": times, "preds_traj": preds_traj, "times_traj": times_traj, "mask": mask,
                ...
                "uncertainty_pred": uncertainty_pred, "uncertainty_embedding": uncertainty_embedding,
        ...
        for dim_to_plot in range(Y_sample.shape[-1]):
            ...
            if self.uncertainty_mode:
                ...
                fig.add_trace(go.Scatter(x=rec_span.tolist() + rec_span.tolist()[
                                                               ::-1], y=upper_bound + lower_bound[::-1], fill="toself",
                                         name='reconstruction uncertainties'))
                fig.add_trace(go.Scatter(x=times_traj.cpu(),
                                         y=uncertainty_traj[0, :, dim_to_plot].cpu(), mode='lines',
                                         name='uncertainty preds'))
    ...
    def add_model_specific_args(cls, parent):
        ...
        parser.add_argument('--predict_from_cn', type=str2bool, default=False,
...
class CNODExtClassification(pl.LightningModule):
    ...
    def configure_optimizers(self):
        return torch.optim.Adam(self.classif_model.parameters(), lr=self.hparams.lr,
