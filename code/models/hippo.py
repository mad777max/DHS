...
class HIPPOmod(nn.Module):
    def __init__(self, Nc, input_dim, Delta, direct_classif, full_mask=False, **kwargs):
        ...
        if not self.direct_classif:
            self.memory_cell = MemoryCell(input_size=input_dim, hidden_size=self.hidden_size, memory_size=self.Nc,
                                          memory_input_size=self.input_dim, full_mask=full_mask)
    ...
    def update_memory_wrap(self, m, u, t0, t1):
        if len(t0.shape) == 2:
            return torch.cat(
                [self.update_memory(m[:, i_dim, :][:, None, :], u[:, i_dim][:, None], t0[:, i_dim], t1[:, i_dim]) for
                 i_dim in range(t0.shape[1])], 1)
        else:
    ...
    def forward_hippo_rnn(self, times, Y, mask, eval_mode=False):
        ...
        for i_t, time in enumerate(times):
            ...
            if i_t == len(times) - 1:
                ...
                if len(mask.shape) == 3:
                    next_t = torch.stack(
                        [times[mask[:, i_t + 1:, dim].argmax(1) + i_t + 1] for dim in range(mask.shape[-1])], -1)
                else:
                    next_t = times[mask[:, i_t + 1:].argmax(1) + i_t + 1][:, None]

            delta_next = next_t - t1[:, None]  # time to next observation
            outputs, state = self.memory_cell(Y[:, i_t, :], state, t0, t1, delta_next, mask[:, i_t],
                                              update_memory_fun=self.update_memory)
...
class HIPPO(pl.LightningModule):
    def __init__(
        ...
        self.hippo_model = HIPPOmod(
            Nc=hidden_dim, input_dim=output_dim, hidden_dim=hidden_dim, Delta=Delta, direct_classif=direct_classif,
            full_mask=full_mask, **kwargs)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {"Y": Y, "preds": preds, "T": times, "mask": mask, "label": label, "pred_class": preds,
                "cn_embedding": cn_embedding}
        ...
        if not self.regression_mode:
            if self.direct_classif:
                ...
                recs = [np.polynomial.legendre.legval(
                    ...
                    (cn_embedding[..., out_dim].cpu().numpy() * [(2 * n + 1) ** 0.5 for n in range(Nc)]).T) for out_dim
    ...
    def predict_step(self, batch, batch_idx):
        ...
        if len(mask.shape) == 3:
            return {"Y_future": Y_future, "preds": preds, "mask_future": mask_future, "pred_rec": recs,
                    "Y_rec": Y[:, mask_rec, :], "mask_rec": mask[:, mask_rec, :], "Y": Y, "mask": mask, "labels": label}
        else:
            return {"Y_future": Y_future, "preds": preds, "mask_future": mask_future, "pred_rec": recs,
                    "Y_rec": Y[:, mask_rec, :], "mask_rec": mask[:, mask_rec], "Y": Y, "mask": mask, "labels": label}
