...
class CNODEmod(nn.Module):
    def __init__(self, Nc, input_dim, hidden_dim, Delta, corr_time, delta_t, method="euler", extended_ode_mode=False, output_fun="mlp", bridge_ode=False, predict_from_cn=False, **kwargs):
        ...
        if self.extended_ode_mode:
            self.node = nn.Sequential(
                nn.Linear(Nc, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, Nc))
            if self.output_fun == "from_cn":
                self.out_fun = get_value_from_cn
            else:
                self.out_fun = nn.Sequential(
                    nn.Linear(Nc, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, input_dim))
        elif self.bridge_ode:
            self.bridge_fun = nn.Sequential(nn.Linear(Nc+2 + 2*input_dim, hidden_dim), nn.ReLU(
    ...
    def ode_fun_wide(self, t, cn):
        ...
        cn_ode = torch.matmul(cn[..., 0], self.A.T) + \
            self.B[None, :]*self.out_fun(cn[..., 1])
    ...
    def integrate(self, cn, eval_times, ode_function=None, **kwargs):
        if ode_function is not None:
            h_out = odeint(ode_function, cn, eval_times, method=self.method, options={
                           "step_size": self.delta_t})
            return h_out,  None
        else:
            if self.extended_ode_mode:
                cn_ext = torch.stack((cn, cn), -1)
                h_out = odeint(self.ode_fun_wide, cn_ext, eval_times,
                               method=self.method, options={"step_size": self.delta_t})
                return h_out[..., 0], self.out_fun(h_out[..., 1])
            elif self.bridge_ode:
                h_out = odeint(lambda t, h: self.ode_bridge_wide(
                    t, h, t0=kwargs["t0"], t1=kwargs["t1"], y0=kwargs["y0"], y1=kwargs["y1"]), cn, eval_times, method=self.method, options={"step_size": self.delta_t})
                return h_out, None
            else:
                h_out = odeint(self.ode_fun, cn, eval_times, method=self.method, options={
                               "step_size": self.delta_t})
    ...
    def forward_ode(self, end_time, start_time, cn, eval_mode=False, **kwargs):
        if eval_mode:
            ...
            post_indices = torch.where(
                eval_times >= end_time-self.corr_time)[0]
            eval_times_ = torch.cat([eval_times[pre_indices], torch.Tensor(
                [end_time-self.corr_time]).to(end_time.device), eval_times[post_indices]])

            pre_h_index = len(pre_indices)
            #h_out = odeint(self.ode_fun,cn, eval_times_, method = self.method, options = {"step_size":self.delta_t})
            h_out, g_out = self.integrate(
                cn=cn, eval_times=eval_times_, **kwargs)
    ...
    def update(self, end_time, cn, y, y_pred, eval_mode=False, eval_times=None):
        ...
        t1 = end_time
        if eval_mode:
            # if eval_times is not None, we need to provide the eval_times
        ...
        h_out, g_out = self.integrate(cn, eval_times_, ode_function=lambda t, h: self.ode_approx(
            t, h, t0=t0, t1=t1, y=y, y_pred=y_pred))
        return h_out, eval_times_

    def compute_bridge_loss(self, h_vec, bridge_info):
        (ids, ts, ys, mask_ids) = bridge_info
        cn_select = h_vec.gather(
            1, ids[..., 0][..., None].repeat(1, 1, h_vec.shape[-1]).long())
        t0 = ts[..., 0][..., None]
        t1 = ts[..., 1][..., None]
        tmid = ts[..., 2][..., None]
        y0 = ys[..., 0][..., None]
        ...
        pred_bridge = (self.bridge_fun(
            cat_input) * self.distance(tmid, t0, t1) + self.linear(tmid, t0, y0, t1, y1))
        loss_bridge = (pred_bridge[mask_ids == 1] -
                       ymid[mask_ids == 1]).pow(2).mean()
    ...
    def forward(self, times, Y, mask, eval_mode=False, bridge_info=None):
        ...
        for i_t, time in enumerate(times):
            if self.bridge_ode:
                next_obs_time_idx = torch.cat([torch.where(mask[i, i_t:] == 1)[0].min()[
                                              None] for i in range(mask.shape[0])]) + i_t
                y1 = Y[torch.arange(Y.shape[0]), next_obs_time_idx, :]
                t1 = times[next_obs_time_idx][:, None]
                h_proj, eval_times, h_pre, eval_times_post, h_post, g_out = self.forward_ode(
                    time, current_time, h, eval_mode=eval_mode, y0=y0, y1=y1, t0=t0, t1=t1)
                ...
                preds = self.compute_preds(
                    h_proj, g_out, eval_mode=eval_mode, post=True)
                #preds = get_value_from_cn(h_proj)
                uncertainty = self.uncertainty_fun(h_proj)

                # taking care of the observation transition
                pred_pre = self.compute_preds(
                    h_pre, g_out, eval_mode=eval_mode, post=False, update_idx=len(h_proj))
                #pred_pre = get_value_from_cn(h_pre)
                #pred_post = get_value_from_cn(h_proj)

                h_update, update_eval_times = self.update(time, h_pre, Y[:, i_t, :].float(
                ), pred_pre, eval_mode=eval_mode, eval_times=eval_times_post)
                # update only the trajectories with observations
                h = h_update[-1] * mask[:, i_t, None] + \
                    h_proj[-1] * (1-mask[:, i_t, None])
                # h[mask[:,i_t]==1] = h_update[-1][mask[:,i_t]==1] # update only the trajectories with an observation

                if eval_mode:
                    #preds_post = get_value_from_cn(h_post)
                    preds_post = self.compute_preds(
                        h_post, g_out[len(h_proj)+1:], eval_mode=eval_mode, post=True)
                    ...
                    non_corrected_uncertainty.append(
                        torch.cat([uncertainty, uncertainty_post])[1:])
                    non_corrected_times.append(
                        torch.cat([eval_times, eval_times_post])[1:])

                    # discarding the correction time
                    preds_update = get_value_from_cn(h_update[1:])
                    # discarding the correction time
                    eval_times = torch.cat([eval_times, update_eval_times[1:]])

                    assert((eval_times_post == update_eval_times[1:]).all())

                    preds_corrected = preds_post.clone()
                    preds_corrected[:, mask[:, i_t] ==
                                    1] = preds_update[:, mask[:, i_t] == 1]
                    ...
                    preds_g = torch.cat(
                        [g_out[:len(h_proj)], g_out[len(h_proj)+1:]])
                    preds_g = preds_g[1:]
        ...
        if self.bridge_ode:
            ...
            if eval_mode:
                ...
                non_corrected_preds = torch.cat(
                    non_corrected_preds).permute(1, 0, 2)
                non_corrected_uncertainty = torch.cat(
                    non_corrected_uncertainty).permute(1, 0, 2)
...
class CNODE(pl.LightningModule):
    def __init__(
        ...
        **kwargs
        ...
        self.hidden_dim = hidden_dim
        self.node_model = CNODEmod(Nc=hidden_dim, input_dim=1, hidden_dim=hidden_dim, Delta=Delta, corr_time=corr_time, delta_t=delta_t,
    ...
    def get_embedding(self, times, Y, mask, eval_mode=False, bridge_info=None):
        _, cn_embedding, _, _, _ = self(times, Y, mask, eval_mode, bridge_info)
        return cn_embedding

    def compute_loss(self, Y, preds, pred_uncertainty, mask):
        mse = ((preds-Y).pow(2)*mask[..., None]).mean(-1).sum() / mask.sum()
        loglik = ((2*torch.log(2*torch.pi*pred_uncertainty.pow(2)) + (preds-Y).pow(2) /
                  pred_uncertainty.pow(2))*(mask[..., None])).mean(-1).sum() / mask.sum()
    ...
    def training_step(self, batch, batch_idx):
        ...
        preds, embedding, pred_times, pred_uncertainty, loss_bridge = self(
        ...
        if self.bridge_ode:
            ...
            if self.direct_classif:
                ...
                accuracy = accuracy_score(
                    label.cpu().numpy(), preds_class.argmax(-1).cpu().numpy())
    ...
    def validation_epoch_end(self, outputs) -> None:
        ...
        if self.bridge_ode:
            ...
            h_vec, _, _, _, _ = self(
                times, Y_sample, mask, bridge_info=bridge_info)
            ...
            for id_ in range(len(t1)):
                for t in torch.linspace(t0[id_], t1[id_], steps=10):
                    cat_input = torch.cat(
                        (h_vec[0, id_], y0[id_], y1[id_], (t1[id_]-t0[id_])[None], (t-t0[id_])[None]), -1)
                    pred_bridge = (self.node_model.bridge_fun(cat_input) * self.node_model.distance(
                        t, t0[id_], t1[id_]) + self.node_model.linear(t, t0[id_], y0[id_], t1[id_], y1[id_]))
                    times_eval.append(t)
            ...
            recs = np.polynomial.legendre.legval(
                (2/self.Delta)*(rec_span-Tmax) + 1, (embedding.cpu().numpy() * [(2*n+1)**0.5 for n in range(Nc)]).T)

            fig = go.Figure()
            fig.add_trace(go.Scatter(x=T_sample[observed_mask[0]].cpu(
            ), y=Y_sample[0, observed_mask[0], 0].cpu(), mode='markers', name='observations'))
            fig.add_trace(go.Scatter(x=times_eval.cpu(),
                          y=y_eval.cpu(), mode='lines', name='interpolations'))
            ...
            if self.direct_classif:
                ...
                accuracy = accuracy_score(
                    labels.cpu().numpy(), preds.argmax(-1).cpu().numpy())
                self.log("val_acc", accuracy, on_epoch=True)

            preds, embedding, pred_times, non_corrected_preds, non_corrected_times, preds_g, non_corrected_uncertainty = self(
                times, Y_sample, mask, eval_mode=True)
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=pred_times.cpu(), y=preds[0, :, 0].cpu(
            ), mode='lines', name='corrected - predictions'))
            fig.add_trace(go.Scatter(x=non_corrected_times.cpu(
            ), y=non_corrected_preds[0, :, 0].cpu(), mode='lines', name='predictions'))
            fig.add_trace(go.Scatter(x=non_corrected_times.cpu().tolist() + non_corrected_times.cpu().tolist()[::-1], y=(non_corrected_preds[0, :, 0].cpu()+non_corrected_uncertainty[0, :, 0].cpu(
            )).tolist() + (non_corrected_preds[0, :, 0].cpu()-non_corrected_uncertainty[0, :, 0].cpu()).tolist()[::-1], fill="toself", name='predictions uncertainty'))
            fig.add_trace(go.Scatter(x=non_corrected_times.cpu(
            ), y=preds_g[0, :, 0].cpu(), mode='lines', name='g-predictions'))
            fig.add_trace(go.Scatter(
                x=rec_span, y=recs[0], mode='lines', name='polynomial reconstruction'))
            fig.add_trace(go.Scatter(x=T_sample[observed_mask[0]].cpu(
            ), y=Y_sample[0, observed_mask[0], 0].cpu(), mode='markers', name='observations'))
...
class CNODEClassification(pl.LightningModule):
    ...
    def forward(self, times, Y, mask, eval_mode=False):
        _, embedding, _, pred_uncertainty, _ = self.embedding_model(
            times, Y, mask)
